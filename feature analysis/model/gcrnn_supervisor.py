...
class GCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        self.use_curriculum_learning = bool(
            self._model_kwargs.get('use_c0rriculum_learning', False))  # 这个参数是啥作用呢？？？
    ...
    def save_model(self, epoch):
        ...
        作用：保存模型
        """
        if not os.path.exists('models6_dffervar/'): # 不存在此路径，则创建
            os.makedirs('models6_dffervar/')
        ...
        torch.save(config, 'models6_dffervar/epo%d.tar' % epoch) # 保存模型，第一个参数为 要保存的对象；第二个参数为 保存文件名
        self._logger.info("Saved model at {}".format(epoch)) # 保存日志
        return 'models6_dffervar/epo%d.tar' % epoch # 将保存的模型的文件路径返回
    ...
    def load_model(self):
        ...
        assert os.path.exists('models6_dffervar/epo%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load('models6_dffervar/epo%d.tar' % self._epoch_num, map_location='cpu') # 加载模型参数
    ...
    def _setup_graph(self):
        ...
        with torch.no_grad(): # 不记录梯度
            self.gcrnn_model = self.gcrnn_model.eval().to(device) # 打开测试模式
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            #
            # x_truths = np.concatenate(x_truths, axis=1)
            # x_truths = x_truths.reshape(x_truths.shape[0],x_truths.shape[1],666,-1)
            #
            # y_truths = np.concatenate(y_truths, axis=1)  # concatenate on batch dimension
            # y_preds2 = y_preds.reshape(y_preds.shape[0],y_preds.shape[1],y_preds.shape[2],1)
            # out1 = np.concatenate((x_truths[:y_preds.shape[0],:y_preds.shape[1],:,:(x_truths.shape[3]-1)],y_preds2,),axis=3)
            #
            # a = out1.shape[0]
            # b = out1.shape[1]
            # c = out1.shape[2]
            # d = out1.shape[3]
            #
            # out1 = self.standard_scaler.inverse_transform(out1.reshape((-1,d)))
            # out1 = out1.reshape((a, b, c, d))
            #
            # y_preds = out1[...,-1]
            #
            #
            # y_truths2 = y_truths.reshape(y_truths.shape[0],y_truths.shape[1],y_truths.shape[2],1)
            # out2 = np.concatenate((x_truths[:y_truths.shape[0],:y_truths.shape[1],:,:(d-1)],y_truths2,),axis=3)
            # a = out2.shape[0]
            # b= out2.shape[1]
            # c = out2.shape[2]
            # d = out2.shape[3]
            #
            # out2 = self.standard_scaler.inverse_transform(out2.reshape((-1,d)))
            # out2 = out2.reshape((a,b,c,d))
            #
            # y_truths = out2[...,-1]
            ...
            #     y_truths_scaled.append(y_truths[t]) # 恢复为真实的 真实值
            #     y_preds_scaled.append(y_preds[t]) # 恢复为真实的 预测值
    ...
    def evaluate2(self, dataset='val', batches_seen=0):
        """
        作用：计算预测结果的损失
        Computes mean L1Loss
        :return: mean L1Loss
        ...
        with torch.no_grad():
            ...
            作用：计算预测结果的损失
         #y: (batch_size,1)
            #x:(batch_size,time_steps.nodes*features)
            """
            #y: (batch_size,1)
            #x:(batch_size,time_steps.nodes*features)
            ...
            x_truths = np.concatenate(x_truths, axis=0)
            x_truths = x_truths.reshape(x_truths.shape[0],x_truths.shape[1],666,-1)


            y_truths = np.concatenate(y_truths, axis=0)  # concatenate on batch dimension
            y_preds2 = y_preds.reshape(y_preds.shape[0],y_preds.shape[1],1,1)
            out1 = np.concatenate((x_truths[:y_preds.shape[0],:y_preds.shape[1],:1,:],y_preds2,),axis=3)
            # zeros2 = np.ones((y_truths.shape[0], y_truths.shape[1],y_preds.shape[2], (x_truths.shape[3]-1)))
            # out1 = np.concatenate((zeros2, y_preds2,), axis=3)
            ...
            y_truths2 = y_truths.reshape(y_truths.shape[0],y_truths.shape[1],1,1)
            out2 = np.concatenate((x_truths[:y_truths.shape[0],:y_truths.shape[1],:1,:],y_truths2,),axis=3)
            # out2 = np.concatenate((zeros2, y_truths2,), axis=3)
            a = out2.shape[0]
            b= out2.shape[1]
            c = out2.shape[2]
            d = out2.shape[3]
            ...
            #
            #
            # for t in range(out1.shape[0]):
            #     out11 = out1[t]
            #     out22 = out2[t]
            #     a = out11.shape[0]
            #     b = out11.shape[1]
            #     c = out11.shape[2]
            #     out11 = self.standard_scaler.inverse_transform(out11.reshape((-1, c)))
            #     out11 = out11.reshape((a, b, c))
            #     y_preds = out11[..., -1]
            #
            #     a = out22.shape[0]
            #     b = out22.shape[1]
            #     c = out22.shape[2]
            #     out22 = self.standard_scaler.inverse_transform(out22.reshape((-1, c)))
            #     out22 = out22.reshape((a, b, c))
            #
            #     y_truths = out22[..., -1]
            #
            #     y_truths_scaled.append(y_truths) # 恢复为真实的 真实值
            #     y_preds_scaled.append(y_preds) # 恢复为真实的 预测值



            return mean_loss, {'prediction': y_preds_scaled, 'truth': y_truths_scaled}

    def evaluate3(self, dataset='val', batches_seen=0):
        """
        作用：计算预测结果的损失
        Computes mean L1Loss
        :return: mean L1Loss
        ...
        with torch.no_grad():
            ...
            y_truths = np.concatenate(y_truths, axis=1)  # concatenate on batch dimension
            y_preds2 = y_preds.reshape(y_preds.shape[0], y_preds.shape[1], self.num_nodes, self.output_dim)
            # out1 = np.concatenate(
            #     (x_truths[:y_preds.shape[0], :y_preds.shape[1], :, :(x_truths.shape[3] - 1)], y_preds2,), axis=3)
            # # zeros2 = np.ones((y_truths.shape[0], y_truths.shape[1],y_preds.shape[2], (x_truths.shape[3]-1)))
            # # out1 = np.concatenate((zeros2, y_preds2,), axis=3)
            ...
            out1 = self.standard_scaler.inverse_transform(y_preds2.reshape((-1, d)))
            out1 = out1.reshape((a, b, c, d))

            y_preds = out1[..., -1]

            y_truths2 = y_truths.reshape(y_truths.shape[0], y_truths.shape[1],self.num_nodes, self.output_dim)
            # out2 = np.concatenate((x_truths[:y_truths.shape[0], :y_truths.shape[1], :, :(d - 1)], y_truths2,), axis=3)
            # out2 = np.concatenate((zeros2, y_truths2,), axis=3)
            a =  y_truths2.shape[0]
            b =  y_truths2.shape[1]
            ...
            out2 = self.standard_scaler.inverse_transform( y_truths2.reshape((-1, d)))
            out2 = out2.reshape((a, b, c, d))
    ...
    def occlu_sen(self,dataset='val', batches_seen=0):
        ...
        explainer = shap.GradientExplainer(self.gcrnn_model, x)
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            # train_iterator = self._data['train_loader'] # 加载训练数据集，返回的是一个生成器
            losses = [] # 要保存的loss
            criterion = torch.nn.MSELoss()

            start_time = time.time() # 一轮训练的开始时间

            # for x, y in train_iterator: # train_iterator是一个生成器，可以迭代，每次迭代则切片 取一次数据，好处是节约内存啊
