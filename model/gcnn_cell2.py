...
class GCGRUCell(torch.nn.Module):
    def __init__(self, num_units, adj_mx, max_diffusion_step, num_nodes, nonlinearity='tanh',
        ...
        self._gconv1_params = LayerParams(self, 'gconv1')
        self._gconv2_params = LayerParams(self, 'gconv2')
    ...
    def forward(self, inputs, hx):
        ...
        if self._use_gc_for_ru:
            fn1 = self._gconv1 # can use other graph conv?
            fn2 = self._gconv2
            # fn = self._chebconv  # can use other graph conv?
        # else:
        #     fn = self._fc
        output = torch.tanh(fn1(inputs, hx, output_size, bias_start=1.0))
        feature = torch.sigmoid(fn2(inputs,output_size, bias_start=1.0))
        out = feature * output
        out = torch.tanh(out)
        new_state = output
    ...
    # gcn
    def _gconv1(self, inputs, state, output_size, bias_start=0.0):
        # Reshape input and state to (batch_size, num_nodes, input_dim/state_dim)
        batch_size = inputs.shape[0]
        inputs = torch.reshape(inputs, (batch_size, self._num_nodes, -1))
        state = torch.reshape(state, (batch_size, self._num_nodes, -1))
        inputs_and_state = torch.cat([inputs[:,:,:-1], state], dim=2)
        input_size = inputs_and_state.size(2)
        ...
        weights = self._gconv1_params.get_weights((input_size * num_matrices, output_size))
        x = torch.matmul(x, weights)  # (batch_size * self._num_nodes, output_size)

        biases = self._gconv1_params.get_biases(output_size, bias_start)
        x += biases
        # Reshape res back to 2D: (batch_size, num_node, state_dim) -> (batch_size, num_node * state_dim)
        return torch.reshape(x, [batch_size, self._num_nodes * output_size])

    def _gconv2(self, inputs, output_size, bias_start=0.0):
        # Reshape input and state to (batch_size, num_nodes, input_dim/state_dim)
        batch_size = inputs.shape[0]
        inputs = torch.reshape(inputs, (batch_size, self._num_nodes, -1))

        inputs_and_state = inputs[:,:,-1:]
        input_size = inputs_and_state.size(2)
        ...
        weights = self._gconv2_params.get_weights((input_size * num_matrices, output_size))
        x = torch.matmul(x, weights)  # (batch_size * self._num_nodes, output_size)

        biases = self._gconv2_params.get_biases(output_size, bias_start)
        x += biases
        # Reshape res back to 2D: (batch_size, num_node, state_dim) -> (batch_size, num_node * state_dim)
        return torch.reshape(x, [batch_size, self._num_nodes * output_size])
