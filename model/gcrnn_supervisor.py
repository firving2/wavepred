...
class GCRNNSupervisor:
    ...
    def save_model(self, epoch):
        ...
        作用：保存模型
        """
        if not os.path.exists('models36z/'): # 不存在此路径，则创建
            os.makedirs('models36z/')
        ...
        torch.save(config, 'models36z/epo%d.tar' % epoch) # 保存模型，第一个参数为 要保存的对象；第二个参数为 保存文件名
        self._logger.info("Saved model at {}".format(epoch)) # 保存日志
        return 'models36z/epo%d.tar' % epoch # 将保存的模型的文件路径返回
    ...
    def load_model(self):
        ...
        assert os.path.exists('models36z/epo%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load('models36z/epo%d.tar' % self._epoch_num, map_location='cpu') # 加载模型参数
    ...
    def occlu_sen(self, dataset='change', batches_seen=0):
        """
        作用：计算预测结果的损失
        Computes mean L1Loss
        :return: mean L1Loss
        ...
        with torch.no_grad():
            ...
            val_iterator = self._data['test_loader'] # 加载验证数据集，这是个迭代器
            losses = []
            ...
            y_truths = np.concatenate(y_truths, axis=1)  # concatenate on batch dimension
            y_preds = y_preds.reshape(y_preds.shape[0],y_preds.shape[1],y_preds.shape[2],1)
            out1 = np.concatenate((x_truths[:y_preds.shape[0],:y_preds.shape[1],:,:(x_truths.shape[3]-1)],y_preds,),axis=3)
            ...
            y_truths = y_truths.reshape(y_truths.shape[0],y_truths.shape[1],y_truths.shape[2],1)
            out2 = np.concatenate((x_truths[:y_truths.shape[0],:y_truths.shape[1],:,:(d-1)],y_truths,),axis=3)
            a = out2.shape[0]
            b= out2.shape[1]
            c = out2.shape[2]
            d = out2.shape[3]
            ...
            return  {'prediction': y_preds_scaled, 'truth': y_truths_scaled}
