...
from model.gcrnn_cell import GCGRUCell
...
def count_parameters(model):
    ...
    """
    return sum(p.numel() for p in model.parameters() if p.requires_grad) # p.numel意思为：p中元素的数量
...
class Seq2SeqAttrs:
    def __init__(self, adj_mx, **model_kwargs):
        ...
        self.rnn_units = int(model_kwargs.get('rnn_units')) # units就是cell中输出层的维度
...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        ...
        self.gcgru_layers =nn.ModuleList(
            [GCGRUCell(self.rnn_units, adj_mx, self.max_diffusion_step, self.num_nodes,
    ...
    def forward(self, inputs, hidden_state=None):
        ...
        hidden_states = []
        output = inputs
        # for layer_num, gcgru_layer in enumerate(self.gcgru_layers):
        #     next_hidden_state = gcgru_layer(output, hidden_state[layer_num])
        ...
        for layer_num, gcgru_layer in enumerate(self.gcgru_layers):
            ...
            hidden_states.append(next_hidden_state)
            output = next_hidden_state
            # output = output.reshape((1,output.shape[0],-1))
            # print(output.shape)
            # print(torch.stack(hidden_states).shape)

        return output,torch.stack(hidden_states)

class DecodeModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        nn.Module.__init__(self)
        Seq2SeqAttrs.__init__(self, adj_mx, **model_kwargs)
...
class GCRNNModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, logger, **model_kwargs):
        ...
        self.decoder_model = DecodeModel(adj_mx, **model_kwargs)
        self.cl_decay_steps =int(model_kwargs.get('cl_decay_steps', 1000))
        self.use_curriculum_learning = bool(model_kwargs.get('use_curriculum_learning', False))
        self._logger = logger
        self.num = int(model_kwargs.get('num_stations', 1))
    ...
    def forward(self, inputs, labels=None, batches_seen=None):
        ...
        #outputs = outputs[-1,:,502]
        #outputs = outputs[-1, :, self.num]  # outstep, batch_size , node
