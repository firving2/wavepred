...
class GCRNNSupervisor:
    ...
    def save_model(self, epoch):
        ...
        作用：保存模型
        """
        if not os.path.exists('models6z/'): # 不存在此路径，则创建
            os.makedirs('models6z/')
        ...
        torch.save(config, 'models6z/epo%d.tar' % epoch) # 保存模型，第一个参数为 要保存的对象；第二个参数为 保存文件名
        self._logger.info("Saved model at {}".format(epoch)) # 保存日志
        return 'models6z/epo%d.tar' % epoch # 将保存的模型的文件路径返回
    ...
    def load_model(self):
        ...
        assert os.path.exists('models6z/epo%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load('models6z/epo%d.tar' % self._epoch_num, map_location='cpu') # 加载模型参数
