...
def shap1(args):
    for ff in range(666):
        ...
        with open(args.config_filename) as f:
            ...
            checkpoint = torch.load('models12z/epo596.tar', map_location='cpu')  # 加载模型参数
            ...
            for j in range(4):
                ...
                for lat_idx, lat in enumerate(latitude):
                    ...
                    for lon_idx, lon in enumerate(longitude):
                        ...
                        # lat = round(lat,2)
                        # lon = round(lon,2)
                        if any((abs(lat - lat_val) < tolerance) and (abs(lon - lon_val) < tolerance) for
                               lat_val, lon_val in
            ...
            # print(ss)
            np.save('shap_values_12h' + str(ff + 1) + 'station', shap_values)
...
#             gcrnn_model = gcrnn_model.to(device)
#             setup_graph(gcrnn_model, data)
#             checkpoint = torch.load('models6z/epo275.tar', map_location='cpu')  # 加载模型参数
#             gcrnn_model.load_state_dict(checkpoint['model_state_dict'])  # 将模型参数 加载到模型中
#
#             gcrnn_model = gcrnn_model.eval()
#             x = data['x_train']
...
#             for j in range(4):
#
#                 swh2 = np.zeros((100 * 6, len(latitude), len(longitude)))  # 创建新的swh2数组
#                 swh3 = np.zeros((100 * 6, len(latitude), len(longitude)))  # 创建新的swh3数组
#
#                 ss = shap_values[:, :, j]
#                 # xx = x_test[:,:,j]
#                 i = 0
...
#             # ss = np.mean(ss,axis=1)
#             # print(ss)
#             np.save('shap_values_6h'+str(i+1)+'station', shap_values)
#
#
#
#
